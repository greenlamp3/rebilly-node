 ...
 */
module.exports = function rebillyMethod(spec) {
  ...
  return function() {
    ...
    for (var i = 0, l = urlParams.length; i < l; ++i) {
      ...
      if (!arg) {
        ...
        throw new Error('Rebilly: I require argument "' + urlParams[i] + '", but I got: ' + arg);
      }
    ...
    if (args.length) {
      throw new Error(
        'Rebilly: Unknown arguments (' + args + '). Did you mean to pass an options object? '
      );
    }
    ...
    // console.log('opts.auth:' + opts.auth);
